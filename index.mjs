...
const manifest = JSON.parse(
  fs.readFileSync("./data/Manifest.ocf.json", "utf8")
);
const stakeholders = JSON.parse(
  fs.readFileSync("./data/Stakeholders.ocf.json", "utf8")
);
const stockClasses = JSON.parse(
  fs.readFileSync("./data/StockClasses.ocf.json", "utf8")
);
const transactions = JSON.parse(
  fs.readFileSync("./data/Transactions.ocf.json", "utf8")
);
...
const valid_tx_stock_issuance = (context, event) => {
  ...
  stakeholders.items.forEach((ele) => {
  ...
  stockClasses.items.forEach((ele) => {
